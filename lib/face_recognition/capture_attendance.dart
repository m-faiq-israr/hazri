...
class CaptureAttendance extends StatefulWidget {
  ...
  const CaptureAttendance({Key key, @required this.teacherId}) : super(key: key);
...
class _CaptureAttendanceState extends State<CaptureAttendance>
                  ...
                  builder: (BuildContext context) {
                    return AlertDialog(
                      ...
                      actions: [
                        ...
                        TextButton(
                          onPressed: () async {
                            await markAttendance(widget.teacherId, 'SE-312', uniqueRecognizedFaceNames);
                            uniqueRecognizedFaceNames.clear();
                           _initializeCamera();
                            Navigator.pop(context);
                          },
  ...
  Future<void> markAttendance(String teacherId, String courseCode, List<String> studentPresent) async {
  try {
    // Check if the provided teacherId matches the teacher field in the course document
    DocumentSnapshot<Map<String, dynamic>> courseDoc = await FirebaseFirestore.instance
    ...
    if (courseDoc.exists) {
      ...
      if (assignedTeacher == teacherId) {
        ...
        print('Attendance Initiated successfully for $formattedDate, Section: $section');

        // Initialize the attendanceList with all students and default status
        List<String> allStudents = ['MARYAM - 74', 'FARZAM - 86', 'FAIQ - 95', 'ZEESHAN - 96'];
