...
class AttendanceScreenState extends State<AttendanceScreen> {
 ...
 // Function to map Firebase data to table rows
  List<Map<String, dynamic>> mapFirebaseDataToTableRows(Map<String, dynamic> attendanceData) {
  ...
  List<Map<String, dynamic>> mapAttendanceListToTableRows(Map<String, dynamic> attendanceData) {
        List<Map<String, dynamic>> rows = [];

        // Extract the attendanceList from the attendanceData
        Map<String, dynamic> attendanceList = Map<String, dynamic>.from(attendanceData['attendanceList'] ?? {});

        // Map attendanceList data to table rows
        attendanceList.forEach((String student, dynamic attendanceStatus) {
          rows.add({
            "name": student,
            "status": attendanceStatus,  // Assuming "status" is the key in your table data for attendance status
            // Add other relevant fields here
          });
        });
  ...
  Map<String, Object?> constructTableData(List<Map<String, dynamic>> rows, String section, String date, String courseCode) {
      return {
    "column_count": null,
    "row_count": null,
    "addable": true,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          GestureDetector(
            ...
            onTap: () async {
              ...
              if (!editing) {
                print('table filling status: ${_editableTableKey.currentState?.currentData.isFilled}');
                final currentData = _editableTableKey.currentState?.currentData;
                  ...
                  if (currentData != null) {
                    ...
                    List<Map<String, dynamic>> previousRows = mapFirebaseDataToTableRows(firebaseDocument!);
      ...
      floatingActionButton: FloatingActionButton(backgroundColor: AppColors.secondaryColor,
      ...
      body: FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
       ...
       /* List<String> absentStudents = List<String>.from(attendanceDataMap['absentStudents'] ?? []);
        ...
        Map<String, dynamic> attendanceList = Map<String, dynamic>.from(attendanceDataMap['attendanceList'] ?? {});
        ...
        List<Map<String, dynamic>> tableRows = mapAttendanceListToTableRows(attendanceDataMap);
        ...
        final data = constructTableData(rows, section, formattedDate, courseCode);
        return SingleChildScrollView(padding: EdgeInsets.all(12.0) ,
          child: Center(
            child:
              EditableTable(
